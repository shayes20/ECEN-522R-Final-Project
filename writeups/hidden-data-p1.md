...
is coming from the source `1.8.1`, and thus we can prepare a filter on that source address.
...
* The `-r` flag specifies the file, in this case `hidden-data-p1.pcapng`.
* The `-Y` flag specifies filters, in this case its packets with source `1.8.1`.
...
* Finally, `>` means to save output of the command to a file, in this case `hidden-data-p1.txt`.
